...
def load_and_compute_ddtf(filepath, p=7, sampling_freq=500.0):
    """
    ...
    cache_ddtf = f"{cache_dir}/{stem}_ddtf.npy"
    cache_ch = f"{cache_dir}/{stem}_channels.npy"
    
    if os.path.exists(cache_ddtf) and os.path.exists(cache_ch):
        return np.load(cache_ddtf), np.load(cache_ch, allow_pickle=True)
    ...
    np.save(cache_ch, np.array(ch_names, dtype=object))
